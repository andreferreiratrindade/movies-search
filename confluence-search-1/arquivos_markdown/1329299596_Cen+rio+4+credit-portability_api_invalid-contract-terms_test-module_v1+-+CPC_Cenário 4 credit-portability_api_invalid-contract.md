---
                            title: "Cenário 4: credit-portability_api_invalid-contract-terms_test-module_v1 - CPC - Área do Desenvolvedor - Open Finance Brasil - Área do Desenvolvedor"
                            paginaId: "1329299596_Cen+rio+4+credit-portability_api_invalid-contract-terms_test-module_v1+-+CPC"
                            url: "https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/1329299596/Cen+rio+4+credit-portability_api_invalid-contract-terms_test-module_v1+-+CPC"
                            source: "Open Finance Brasil Confluence"
                            ---

                            # Cenário 4: credit-portability_api_invalid-contract-terms_test-module_v1 - CPC - Área do Desenvolvedor - Open Finance Brasil - Área do Desenvolvedor

                            Tipo do teste
Teste de exceção
Descrição
Validação de mensagens e comportamentos do sistema em situações de inconsistências no contrato de verificação de respostas.
Objetivo
Garante que o pedido de portabilidade seja rejeitado quando há inconsistências no contrato
Requisito de massa
...
Status 422 - Validação de resposta e código de erro esperado como 'CAMPO_INCONSISTENTE'
Etapa de criação de solicitação de portabilidade
10
Chamada do endpoint POST /portabilities enviando os seguintes valores:
data.institution.creditor.companyCnpj = original companyCnpj
instalmentPeriodicity ≠ different from original instalmentPeriodicity
proposedContract.totalNumberOfInstalments = original dueInstalments
contractAmount = original contractOutstandingBalance
Status 422 - Validação de resposta e código de erro esperado como 'PERIODICIDADE_INVALIDA'
11
Chamada do endpoint POST /portabilities enviando os seguintes valores:
data.institution.creditor.companyCnpj = original companyCnpj
instalmentPeriodicity = original instalmentPeriodicity
proposedContract.totalNumberOfInstalments > grater then original dueInstalments
contractAmount = original contractOutstandingBalance
Status 422 - Validação de resposta e código de erro esperado como 'PRAZO_ACIMA_LIMITE'
12
Chamada do endpoint POST /portabilities enviando os seguintes valores:
data.institution.creditor.companyCnpj = original companyCnpj
instalmentPeriodicity = original instalmentPeriodicity
proposedContract.totalNumberOfInstalments = original dueInstalments
contractAmount > grater then original contractOutstandingBalance
Status 422 - Validação de resposta e código de erro esperado como 'NAO_INFORMADO'
13
Fim do teste
