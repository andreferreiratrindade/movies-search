---
                            title: "FAQ - JSR - v2.2.0 - [SV] Vínculo de dispositivo - Área do Desenvolvedor - Open Finance Brasil - Área do Desenvolvedor"
                            paginaId: "1436516567_FAQ+-+JSR+-+v2.2.0+-+SV+V+nculo+de+dispositivo"
                            url: "https://openfinancebrasil.atlassian.net/wiki/spaces/OF/pages/1436516567/FAQ+-+JSR+-+v2.2.0+-+SV+V+nculo+de+dispositivo"
                            source: "Open Finance Brasil Confluence"
                            ---

                            # FAQ - JSR - v2.2.0 - [SV] Vínculo de dispositivo - Área do Desenvolvedor - Open Finance Brasil - Área do Desenvolvedor
...
Para o endpoint /consents/{consentId}/authorise, deve rejeitar a solicitação com o código de erro ORIGEM_FIDO_INVALIDA.
Para o endpoint /recurring-consents/{recurringConsentId}/authorise, deve rejeitar a solicitação com o código de erro ORIGEM_FIDO_INVALIDA.
Fluxo
Operação
...
1.2 Certificação Funcional Pagamentos (API Pagamentos e/ou API Pagamentos Automáticos) via Open Finance;
...
1.4.2 Se a instituição não possui ou utiliza FIDO Server certificado, a instituição deve obtê-las conforme passos descritos nesse link (
...
Sim. Com base na Instrução Normativa,
https://www.bcb.gov.br/estabilidadefinanceira/exibenormativo?tipo=Resolu%C3%A7%C3%A3o%20BCB&numero=406
. Essa Instrução Normativa apresenta as datas da obrigatoriedade.
Regulatório
Obrigatoriedade
...
Sim. o próprio motor utiliza o mockback.
Outros
Qual seria a orientação para uma instituição participar de forma voluntário do piloto da Jornada Sem Redirecionamento?
A instituição pode entrar em contato diretamente com o BC e com a DTO pelos e-mails
openfinance.denor@bcb.gov.br
,
gp_aceleracao_interoperabilidade@openfinancebrasil.org.br
e
gp_servicos@chicagoadvisory.com.br
,  indicando que tem interesse em participar do Piloto de forma voluntária como detentora de conta.
Outros
...
Inicialmente, deve ser apresentado ao usuário com o campo não preenchido;
...
O cliente deve ter opção de inserir um valor;
O valor eventualmente inserido não deve ser submetido a consistência quanto aos limites referenciados no item 2
Caso o usuário não defina um limite, a instituição detentora de conta deverá considerar que o limite diário é o limite do Pix;
Caso um limite não tenha sido definido, não se deve considerar que o limite é 0 e rejeitar todos as tentativas de pagamento
Para o limite por transação:
Deve ser apresentado, por padrão, R$ 500,00
O cliente deve ter opção de reduzir o valor
Caso haja contrato bilateral entre a iniciadora e a detentora, o valor padrão pode ser maior que R$500,00 (a depender do contrato bilateral);
Limites
